...
- This box has NO audio output endpoint (CoreAudio GetMute → 0x80070490), so muteWhileRecording cannot be asserted here; query script kept at `C:\Users\Administrator\tts\getmute.ps1` for a real machine. keepFailedAudio pruning only fires when a finish-stage recognition failure persists audio — needs fault injection (temporarily corrupt a model file), not network blocking.
