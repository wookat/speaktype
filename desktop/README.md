...
- **语音识别**：服务商可插拔——豆包（登录并用一次它自带的语音输入即自动获取 App Key，也可手填）/ 任意 OpenAI 兼容转写接口（内置多家预设 + 测试连接）/ 内置离线识别（whisper.cpp，应用内下载 tiny/base/small 模型，可选强制简体输出）；可选开启内置的 Silero VAD 升级人声检测。
