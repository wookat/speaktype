...
开源（MIT）的 Windows AI 语音输入法：Electron 三窗口（主界面 / 贴底悬浮波形条 / toast）+ 全局热键 + 任意 Windows 程序落字。SpeakType 自身不提供任何云端服务：语音识别走你自己登录的豆包会话，AI 润色接你自己配置的任意 OpenAI 兼容模型，所有配置与密钥只存本机。
...
- **语音识别**：登录豆包并用一次它自带的语音输入即自动获取 App Key，也可手动填写；识别语言可选。
