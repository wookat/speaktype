# SpeakType Chrome 浏览器扩展

SpeakType 的早期形态：在网页内按住说话、松手落字的 Chrome MV3 扩展，与[桌面版](../desktop/README.md)共享豆包协议层。代码在仓库根目录（`entrypoints/`、`lib/` 等）。你说，我写。在任意网页的输入框里**按住 `Ctrl` 说话、松手落字**（也可按 `Alt+Q` 或点悬浮条按钮开/关），自动转写 + 按场景改写，直接落到光标处。
