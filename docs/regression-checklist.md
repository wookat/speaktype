...
- [ ] 识别语言下拉恰好 6 项（自动/中/英/日/韩/粤），首项「自动检测」（第 2/159 轮）
...
## 失焦即结算 + 手机麦入口（第 37 轮固化）
- [ ] 落字后立刻改词、不等 1.5 秒停顿直接点击别的窗口 → 改动立即结算学到（log 出现 auto-learn，无需等观察进程退出）
- [ ] Home 引导卡下方出现「用手机当麦克风 →」链接，点击跳设置→通用（含手机麦克风区块）
- [ ] 日文 Undo 点击后确认 toast 文案为「元に戻しました」
