...
## 第 280 轮补测（同环境续测，被测打包版仍为 main @ 45d8b17 构建产物，无代码变更）

- 日期：2026-09-01
- 范围：手机页五语言走查、手机页切后台 30s 恢复、RightCtrl 轻量回归（双手机抢占专项已在上方「第 280 轮」小节完成并立案 280-P3-1，此处不重复）
...
- 房间页 `https://speaktype.zalize.com/relay/m/<room>?lang=en|zh-CN|zh-TW|ja|ko` 五口径逐一加载（CDP 抓取 innerText + UTF-8 字节核验）：标题、连接状态、「按住说话」主按钮、添加到主屏幕、断开这台电脑、页脚说明均正确本地化，无截断、无乱码、无缺翻译（en「Hold to talk」/ zh-CN「按住说话」/ zh-TW「按住說話」/ ja「押しながら話す」/ ko「누르고 말하기」）。
- 错误/提示文案覆盖核验：页面内嵌 i18n 串逐条检查，五语言均包含完整错误提示集——等待电脑（「已连接中转，等待电脑…」等）、房间被占（「该房间已有手机连接」/「このルームには既に別のスマホが接続しています」/「이 방에는 이미 다른 폰이 연결되어 있습니다」）、配对失效、断线重连中、电脑离线、电脑正在录音、配对码格式错误、扫码不支持、相机/麦克风权限被拒，全部已翻译且无乱码（UTF-8 字节实证）。
- 渲染抽查：zh-CN（ss_3bad92d5）与 ko（ss_66709b51）真实窗口截图，主按钮圆形区域文字完整居中显示，无溢出截断。
- 备注：等待电脑状态用不存在房间号实测五语言各一遍（实测）；其余错误态（权限被拒、配对失效等）为页面内嵌 i18n 字符串字节核验，未逐一构造真实错误场景触发（推断其显示路径与已实测文案一致）。

### 2. 手机页切后台/最小化 30s 恢复 ✅（实测）

- CDP 将手机页窗口最小化 32s 后恢复：页面仍显示「Connected to your PC」，连接未断（实测）。
- 恢复后立即按住说话 8s：整句中文经手机链路完整落入记事本（finalize 20:28:07，ss_90f7026f），无需刷新、无错误提示。
- 判断：切后台 30s 场景连接自动保持，行为合理，不立案。真机移动端息屏（OS 冻结 WebSocket/页面）与桌面窗口最小化机制不同，列入未测试项。

### 3. 轻量回归：RightCtrl 中文落字 ✅（实测）

- 按住 8s，整句中文完整追加落入记事本（Col 29→57，finalize 20:28:50，ss_462b5b12），无回归。

### 第 280 轮补测实测/推断区分

- 五语言主流程文案、后台恢复、RightCtrl 均为打包版+线上手机页实测；错误态文案的完整性为线上页面 i18n 字符串字节核验（实证存在且正确），其真实触发路径未逐一实测（推断）。

### 第 280 轮补测未测试项

- 真机移动端息屏/锁屏 30s+（OS 级挂起，桌面最小化不能完全等价模拟）。
- 各错误态（权限被拒、配对失效等）的真实触发式五语言显示。

### 第 280 轮补测清理

已退出 SpeakType 与记事本（不保存）、关闭手机模拟 Chrome 实例（9444 端口确认关闭）；未修改产品代码。
